...
```
User  ──(member of 1+)──▶  Account  ──(has)──▶  Instance
                           (tenant)             (1 engine
                                                 deployment)
```
...
- **Platform Link**: Twitch OAuth tokens (and future platforms), scoped to an Instance.

Almost everything in the UI is scoped to the currently selected Instance. An engine is single-tenant: it holds one set of data, so engine calls carry no tenant or application id — the credentials a request authenticates with already identify the engine. Behaviour that differs by platform or channel belongs in workflow trigger conditions, not in a scoping id.
...
`instanceId` is the Convex `_id` of the instance record. Convex is its only authority. It identifies which engine deployment a request or callback belongs to; Convex routes an incoming callback to its instance by the callback's Bearer token. The engine has no id of its own to hand back.
...
| Proxied requests and callbacks carry no tenant or application id | Each engine is single-tenant; its credentials already identify it |
...
`@woofx3/api` resolves to a sibling `woofx3` checkout and gives both Convex and client code access to the engine's shared type package (`Woofx3EngineGateway`, `Woofx3EngineApi`, etc.). `convex/lib/engineInstanceUrl.ts` bridges the shared shape into capnweb's proxy typing via `interface EngineApi extends RpcTarget, Woofx3EngineApi {}`. Per-file RPC interfaces (e.g. `WoofxEngineRpc` in `workflowCatalog.ts`) still exist and will be retired as the shared type package catches up to what the engine exposes.
